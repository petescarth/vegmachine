...
L.TimeDimension.Layer.WMS = L.TimeDimension.Layer.extend({
    ...
    _onNewTimeLoading: function(ev) {
        // console.log('Layer._onNewTimeLoading: ' + this._baseLayer.wmsParams.layers + ' with time: ' + new Date(ev.time).toISOString.split('T')[0]);
    ...
    _showLayer: function(layer, time) {
        ...
        console.log('Show layer ' + layer.wmsParams.layers + ' with time: ' + new Date(time).toISOString().split('T')[0]);
    ...
    _getLayerForTime: function(time) {
        ...
        newLayer.on('load', (function(layer, time) {
            ...
            // console.log('Loaded layer ' + layer.wmsParams.layers + ' with time: ' + new Date(time).toISOString().split('T')[0]);
            this.fire('timeload', {
                time: time
            });
    ...
    _createLayerForTime:function(time){
        ...
        wmsParams.time = new Date(time).toISOString().split('T')[0];
    ...
    _getNearestTime: function(time) {
        ...
        if (time != this._availableTimes[index]) {
            console.log('Search layer time: ' + new Date(time).toISOString().split('T')[0]);
            console.log('Return layer time: ' + new Date(this._availableTimes[index]).toISOString().split('T')[0]);
        }
